...
*/
const DEV_LINKS = [
  { icon:"heart", title:"Support Me", sub:"Dukung ZNAL AI lewat Sociabuzz", url:"https://sociabuzz.com/nalaneo_official/tribe" },
  ...
  { icon:"instagram", title:"Instagram", sub:"@nalaneo", url:"" },
  ...
  */
  ...
  if(isPythonLang){
    ...
    const runNow = () => {
      ...
      if(isPdfGen){
        runPdfOnServer(block.code, block.filename, resultEl);
      }else{
  ...
  if(!pyodideLoading){
    pyodideLoading = (async () => {
      try{
        ...
        // fpdf2 sengaja TIDAK di-install lagi — PDF sekarang dibuat di
        // server (ReportLab) lewat runPdfOnServer(), bukan di browser.
...
if(redeemSubmitEl){
  redeemSubmitEl.onclick = () => {
    ...
    const expiresAt = Date.now() + found.validityDays * 24 * 60 * 60 * 1000;
    localStorage.setItem("znal_redeemed_" + userKey, JSON.stringify({ code, type: found.type, redeemedAt: Date.now(), expiresAt }));
    localStorage.removeItem("znal_usage_" + userKey);
    updateLimitBanner();
    updateRedeemUsageBar();

    redeemStatusEl.textContent = "Berhasil! " + found.label + " aktif sekarang 🎉";
    redeemStatusEl.className = "ok";
    redeemCurrentEl.textContent = redeemStatusLabel(found);
